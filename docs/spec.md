...
**セッションを切らさないための扱い**（`src/lib/auth-session.ts`）。

- `supabase.auth.getUser()` は Supabase への HTTP リクエストなので、圏外・一時障害・429 でも user は null になる。
  `judgeAuth()` で **`unauthenticated`（本当に未ログイン）と `unverified`（確認できなかった）を分け**、
  `unverified` はログイン画面へ飛ばさずに通す。`(app)` レイアウトは代わりに「接続を確認できませんでした／
  ログアウトはしていません」（`ConnectionLost`）を出す。歩きながら使うアプリで、電波の切れ目が
  ログアウトに化けるのを防ぐため
- **認証クッキーは proxy が毎リクエスト、サーバから打ち直す**（`Max-Age` 400日）。`@supabase/ssr` の
  ブラウザ側クライアントはセッションを `document.cookie` に書くが、Safari は「Safari を7日使う間
  そのサイトに触れていない」と JS が書いたストレージを消す。サーバの `Set-Cookie` で上書きし続けて
  寿命を先へ転がす。`code-verifier` は使い捨てなので対象外。リダイレクトを返すときは
  Supabase が書いたクッキーを新しいレスポンスへ載せ替える（落とすと無効なトークンが残り、毎回弾かれ続ける）
- **ログインの失敗理由は必ず画面に出す**。`/auth/callback` は `?error=verify|expired|link` を付けて
  `/login` に戻し、ログイン画面がそれを日本語で説明する。無言でフォームに戻すと「メールが効かない」と見えて
  送り直しを誘発し、内蔵メールの上限（1時間に2通）に当たる。とくに **`AuthPKCECodeVerifierMissingError`
  ＝リンクを送ったブラウザと別のブラウザで開いた**ときは、そう言わないと直しようがない
...
- **ドリル中の文法解説**：フッターの戻りボタンの右に本のアイコン（`BookOpen`）を置き、いま表示している文の「なぜこの日本語がこの英語になるか」を AI（`POST /api/ai/grammar`）に書かせてプレイヤー全体に被せて出す。動画で ChatGPT にやらせていた問い直しを、ドリルを離れずに済ませるための導線（ワークスペースの `ExplainPanel` が自由質問なのに対し、こちらは**質問文を打たずに1タップ**で出るのが要点。歩きながらでも押せる）。
  - **押すと必ず一時停止する**。解説の生成に10〜30秒かかるので、止めないと読んでいるあいだに次の文へ送られる。`pause()` は Wake Lock を手放すが、解説は読み物なので**開いているあいだだけ取り直す**（読んでいる途中で画面が暗くならない）
  - **答えを見たあとにしか押せない**（考える時間中は `disabled`）。答えを見る前に解説を読めてしまうと、自力で英語を作るという肝心の練習が飛ぶ。隠さず無効表示にして、そういうボタンがあること自体は分かるようにする
  - 面はプレイヤー全体を覆う（`absolute inset-0`）ので、開いているあいだは送り・戻り・★・終了に触れない。**閉じても再開はしない**（読み終えた直後に読み上げが走ると面食らうため。再開は「タップで再開」に委ねる）
  - **解説は DB に保存しない**。同じ文への再取得を避けるためタブ内のメモリにだけ持つ（キーは日本語＋英語なので、例文を直せば自然に取り直しになる）。リロードすれば消える
...
**`migration 0011`（応用練習の出自）** — `compositions` に `source text not null default 'manual'`（`check (source in ('manual','ai'))`）を足す。応用練習で作った文は**新テーブルを作らず**ここに入れる（★・読み上げ・回数記録・中断と再開・編集・削除が既存のまま効くため）。既存行は `'manual'` で埋まるので、当てるだけで従来どおり動く。

> **番号が 0010 ではなく 0011 なのは、一度ぶつけたから。** 発音記号と応用練習を別ブランチで並行して作り、どちらも「次は 0010」と判断したまま両方 main に入った。Supabase はファイル名の先頭を `supabase_migrations.schema_migrations` の**主キー**にするので、先にマージされた発音記号が `0010` を埋め、あとから来た応用練習の `supabase db push` が `duplicate key value violates unique constraint "schema_migrations_pkey"` で落ちた。`deploy-production.yml` はマイグレーション成功後にしか Vercel を叩かないため、**列だけでなく応用練習のコードごと本番に出なかった**。番号は各PR単体では正しく、**2本が揃って初めて壊れる**ので、`pr-checks.yml` の `migrations` ステップでディレクトリ全体の番号重複を見張っている。
>
> **本番は `0010`＝発音記号・`0011`＝応用練習で揃っている。** 発音記号は PR #33 のデプロイで `0010` として、応用練習は採番し直したあと PR #35 のデプロイで `0011` として当たった。**`0001`〜`0011` はすべて本番適用済み**（`supabase db push` は未適用ぶんを版番号順にまとめて当てるので、`0007` だけを当てたデプロイのログが「それ以前は全部入っている」ことの証拠になる）。
>
> **ローカルの台帳だけ一時的に本番と逆向きになっていた。** 衝突を解くブランチを2本作ってしまい（発音記号を `0011` へ／応用練習を `0011` へ）、main に入ったのは後者なのに、ローカルへ当てたのは前者だったため `0010`=`composition_source` / `0011`=`clip_ipa` と記録されていた。列は両方入っていたので画面は動くが、**リポジトリの `0010`・`0011` はこのDBに二度と実行されない**状態だった。
>
> **`supabase migration list` では気づけない。** 版番号しか突き合わせないので 0001〜0011 が全部緑に見える。`schema_migrations` の `name` まで見て初めて分かる。2026-09-06 に `0010` と `0011` の `name` / `statements` を入れ替えて解消した（列は両方入っていたので `db reset` は不要で、ローカルの学習データも残した）。**同じ疑いを持ったら `select version, name from supabase_migrations.schema_migrations order by version;` をファイル名と突き合わせる。**
...
| **セッションを切らさない扱い**（`judgeAuth` ／ クッキーの打ち直し ／ 失敗理由の表示） | `next start` で確認。①クッキー無し → `/login?next=/` へ 307 ②セッションのクッキーを持たせて Supabase を落とした状態 → **/login へ飛ばさず 200 で「接続を確認できませんでした」**、同時に `Set-Cookie ... Max-Age=34560000`（400日）がサーバから出ることを確認 ③`/login?error=verify` で「同じブラウザで開く必要があります」が出ることを確認 |
