...
- 例文の CRUD。例文は `ja`（日本語）＋`en`（英語）の対。1件ずつの追加・編集・削除ができる
...
- 開始すると日本語が1文だけ大きく出る → 設定秒だけ「考える時間」 → **答え（英語）を表示し、同時に読み上げる** → 次の文へ。これをコースの文章数だけ繰り返して1周で終わる
- 読み上げは端末の音声合成（Web Speech Synthesis, `en-US`）。次への送りは読み上げ終了（`onend`）を待つので、長い文ほど答えが長く残る（TTS 非対応環境では固定秒で送る）
- 「歩きながら・見ずに口だけ」を許すため Wake Lock を張る（独り言と同じ）。iOS はユーザー操作を起点にしないと発話がブロックされるので、スタート押下時に無音の発話で一度解錠しておく
- 最後の設定（順番・秒数）は localStorage に覚える。DB には持たない

**継続への反映**

- 答えを表示・読み上げた文を **1回** と数え、run 終了時（および途中離脱時）に `composition_logs` へ `rep_count` としてまとめて記録する。聴くだけ・途中離脱でも、そこまでに読み上げた数は残す
