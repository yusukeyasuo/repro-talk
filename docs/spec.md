...
| 続いている記録 | 連続日数、今週の学習時間、今週の独り言の合計時間、今週のリプロダクション回数、今週の瞬間英作文回数 |
...
| ビルド / Lint / ユニットテスト61件 | 全通過（`next build` で `/api/ai/naturalize` 含む全ルート生成、`tsc --noEmit`・eslint クリーン） |

### 未検証

| 対象 | 理由 |
|---|---|
| **学習時間の本番反映**（`migration 0008` のクラウド Supabase 適用） | ローカルには適用・検証済みだが本番へは未適用（CI のマイグレーション→デプロイ順で流す） |
...
`npm test` は `node:test` で `tests/*.test.ts` を実行する（61件）。
